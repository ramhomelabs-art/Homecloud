...
const InspectorSidebar = ({
    ...
    useEffect(() => {
        ...
        if (metadata && !metadata.isDirectory) {
            ...
            if (isImage) {
                ...
                const url = `/api/files/download?path=${encodeURIComponent(metadata.path)}&token=${token || ''}${agentIdQuery}`;
    ...
    const previewUrl = (metadata && isImage) ? `/api/files/download?path=${encodeURIComponent(metadata.path)}&token=${token || ''}${agentIdQuery}` : null;
