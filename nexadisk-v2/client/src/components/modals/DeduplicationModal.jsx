...
const DeduplicationModal = ({ path, agentId, onClose, onRefresh, showToast }) => {
    ...
    const performScan = async () => {
        ...
        try {
            const res = await axios.post(`${API_BASE}/duplicates/scan`, {
                path,
                agentId
            });
            setGroups(res.data || []);
    ...
    const handleDeduplicate = async () => {
        ...
        setDeleting(true);
        try {
            const res = await axios.post(`${API_BASE}/files/delete/batch`, {
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-content glass" onClick={e => e.stopPropagation()} style={{ maxWidth: '800px', width: '90%', maxHeight: '85vh', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ fontSize: '12px', color: '#8b949e', marginBottom: '20px', wordBreak: 'break-all', flexShrink: 0 }}>
                    Scanning Target: <span style={{ color: '#fff', fontWeight: 'bold' }}>{path}</span> {agentId && <span className="partition-badge" style={{ marginLeft: '8px' }}>Remote Node</span>}
                </div>
                ...
                {loading ? (
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, padding: '60px 0' }}>
                        ...
                        <p style={{ marginTop: '16px', fontWeight: '600', color: '#fff' }}>Analyzing directories and files...</p>
                        <p style={{ fontSize: '12px', color: '#8b949e', marginTop: '4px' }}>Calculating size matches and comparing checksum hashes.</p>
                    </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, padding: '40px 0', textAlign: 'center' }}>
                        ...
                        <p style={{ color: '#8b949e', maxWidth: '300px', fontSize: '13px' }}>No duplicate files were found in this directory. Everything is organized.</p>
                    ...
                    <>
                        ...
                        <div style={{ flex: 1, overflowY: 'auto', paddingRight: '4px', marginBottom: '20px' }}>
                            {groups.map((group, groupIdx) => (
                                <div key={groupIdx} style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border-dim)', borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px', borderBottom: '1px solid var(--border-dim)', paddingBottom: '8px' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                            ...
                                            <span style={{ fontSize: '12px', color: '#8b949e' }}>MD5 Hash: {group.hash.slice(0, 8)}...</span>
                                        </div>
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <div style={{ minWidth: 0, flex: 1 }}>
                                                        ...
                                                        <div style={{ fontSize: '11px', color: '#8b949e', marginTop: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{file.path}</div>
                                                    </div>
                                                    <span style={{ fontSize: '11px', color: '#8b949e', flexShrink: 0 }}>Mod: {date}</span>
                                                </div>
                                            );
                                        })}
                                    </div>
                        ...
                        {groups.length > 0 && (
                            <div className="glass" style={{ padding: '16px 20px', borderRadius: '12px', border: '1px solid var(--border-bright)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', flexShrink: 0 }}>
                                <div>
                                    <div style={{ fontSize: '12px', color: '#8b949e' }}>Selected for deletion: <span style={{ color: '#fff', fontWeight: 'bold' }}>{selectedFiles.size} files</span></div>
