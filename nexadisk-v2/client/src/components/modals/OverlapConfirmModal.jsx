            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="modal-content glass" style={{ width: '400px', textAlign: 'center' }}>
                <div style={{ padding: '20px' }}>
                    ...
                    <p style={{ fontSize: '14px', color: '#8b949e', lineHeight: '1.5' }}>
                        An item named <span style={{ color: '#fff', fontWeight: 'bold' }}>"{fileName}"</span> already exists in this folder. Do you want to replace it?
                    </p>
                    <div style={{ display: 'flex', gap: '12px', marginTop: '30px' }}>
                        <button className="btn-secondary" style={{ flex: 1 }} onClick={onClose}>Skip</button>
                        <button className="btn-primary" style={{ flex: 1, background: 'var(--accent-gold)', borderColor: 'var(--accent-gold-glow)' }} onClick={() => onConfirm(context)}>Overwrite</button>
                    </div>
                </div>
            </motion.div>
        </div>
