...
router.post('/create', authenticateToken, requireRole(ADMIN_ROLES), async (req, res) => {
    ...
    try {
        ...
        const passwordHash = password ? encryptPassword(password) : null;
...
router.get('/list', authenticateToken, async (req, res) => {
    try {
        ...
        const shares = r.rows.map(s => ({
            ...
            password: s.password_hash ? decryptPassword(s.password_hash) : null,
...
router.put('/:token', authenticateToken, requireRole(ADMIN_ROLES), async (req, res) => {
    ...
    try {
        ...
        if (password !== undefined) {
            const h = password ? encryptPassword(password) : null;
