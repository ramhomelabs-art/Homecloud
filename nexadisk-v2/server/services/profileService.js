...
const STORAGE_ROOT = '/opt/nexadisk/storage/profiles';
...
class ProfileService {
    ...
    async getUserProfile(userId) {
        const res = await db.query(
            'SELECT id, username, role, display_name, first_name, last_name, email, phone, department, job_title, time_zone, language, bio, account_status, avatar_path, avatar_thumbnail_path, last_login, created_at, avatar_updated_at FROM users WHERE id = $1',
            [userId]
        );
        return res.rows[0];
    }
