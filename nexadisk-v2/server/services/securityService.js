...
const logger = require('../utils/logger'); const yaraService = require('../services/yaraService');
...
class SecurityService {
    async deepScan(filePath, originalName) {
        ...
        if (whitelistExts) {
            ...
            if (list.includes(ext) || list.includes(originalName.toLowerCase())) {
        ...
        try {
            ...
            // Attempt to scan with clamscan. If clamscan is not found, it throws an error and we just catch and ignore.
            const { stdout } = await execPromise(`clamscan --no-summary "${filePath}"`, { timeout: 15000 });
            // Clamscan returns 0 if clean, 1 if virus found. But since execPromise throws on non-zero exit code, 
            // if we are here, it means 0 (Clean).
            ...
            if (err.code === 1 && err.stdout) {
                ...
                // Command not found or timeout, just skip gracefully
    ...
    async _finalizeScan(filePath, fileHash, verdict, score, threats, details, durationMs) {
        ...
        try {
            ...
            if (verdict === 'malicious' || verdict === 'suspicious') {
                ...
                notificationService.sendInAppAlert(title, detail, severity);
            }
        } catch (e) {
